...
- **语义标签标注**：以规则完成候选标签召回，以模型判断评论语境中的优势、问题与混合倾向，降低关键词反义和歧义误标。
- **引导式需求补全**：对已识别品类的预算、关注点与避免项使用前端交互补全，在同一 Agent 任务内恢复流程，减少重复意图识别与模型调用。
- **Agentic Memory**：构建 Ledger、Summary、Profile、Sliding Window 与 Policy 分层记忆，使用户确认的长期偏好能够被按需、安全地调度。
- **生产级访问治理**：补齐 JWT 鉴权、用户与会话隔离、按用户/IP 限流、输入安全校验及越权访问测试。
- **可靠性评测**：建立并发压测、推荐质量评测集、节点耗时与模型成本监控，持续验证系统在真实访问压力下的稳定性。
